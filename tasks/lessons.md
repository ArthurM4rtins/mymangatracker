...
## `head` no fim do comando de verificacao esconde o que ele existe para achar

- **Tentativa**: antes de commitar o conserto da paginacao (#290), rodei `npx tsc --noEmit | head -3; echo "tsc ok"` e segui. O CI reprovou por tres erros de tipo nos testes que eu mesmo tinha escrito — `FiltroDoCatalogo` exige `ordem`, e eu passava so `{ termo }`.
- **Erro**: dois defeitos no mesmo comando. O `head -3` cortou a saida exatamente onde os meus erros estavam, porque os tres primeiros eram de outro arquivo, pre-existentes e de ambiente. E o `echo "tsc ok"` estava depois de `;`, nao de `&&`, entao imprimiu "ok" com o `tsc` saindo em 2. Eu li a propria mensagem que escrevi como se fosse resultado.
- **Regra**: comando de portao — `tsc`, `lint`, `test`, `build` — roda com a saida **inteira** e sem texto proprio de sucesso. Resumir com `tail` e aceitavel porque o erro fica no fim; `head` nao, porque corta justamente o que veio depois do ruido conhecido. Se for para imprimir veredito, que venha do exit code (`&&` ou `|| echo FALHOU`), nunca de um `echo` incondicional — mesma familia do [portoes-nao-leem-a-saida], so que aqui o portao falou e eu tapei o ouvido.
