...
## Varredura de rename tem que cobrir as três caixas do nome

- **Tentativa**: no rebranding, varri o repo com `grep -rn "Kidoku\|kidoku\|既読"` em `src` e `tests`, e com `KIDOKU` incluído só em `extension/`. Lint, `tsc` e os 769 testes ficaram verdes, e a marca sumiu de tudo que eu conferi.
- **Erro**: `KIDOKU_I18N` continuou vivo em `tests/i18n/extensao.test.ts`, porque a caixa alta não estava no padrão usado em `src`/`tests`. E o pior não foi sobrar o nome: aquela string era a **regex de um portão**. Renomeado o global para `FOLUNIO_I18N`, ela parou de casar com nada, e o teste que garante que toda chave pedida pelo popup existe nos cinco catálogos **ficou verde conferindo zero chaves**. Passou por um merge e uma semana assim.
- **Regra**: nome de símbolo vive em três caixas — `Kidoku`, `kidoku`, `KIDOKU` — e a varredura usa as três **em todo o repo**, não uma lista por pasta. E quando o nome aparecer dentro de uma regex, de um seletor ou de uma string que alimenta uma busca, o conserto não é só trocar: é fazer o portão **estourar quando não achar nada**. Portão que não acha nada passa — mesma família do [portoes-nao-leem-a-saida], e o caso mais caro dela, porque o verde mente.
